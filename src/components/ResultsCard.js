...
const ResultsCard = ({ isOpen, closePopup, resetTime, typedChars, typedWords, errors }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 transition-opacity duration-300 ease-in-out">
      <div className="relative w-1/2 h-1/2 p-8 flex flex-col justify-center bg-gradient-to-br from-emerald-50 to-emerald-200 rounded-xl shadow-xl transform scale-95 transition-transform duration-300 ease-in-out">
        <button
          onClick={handleClose}
